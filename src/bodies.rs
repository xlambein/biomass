...
use crate::{
	add_ingredient,
	constants::{
		ASTEROID_ANGULAR_VELOCITY, ASTEROID_DESPAWN_DISTANCE, ASTEROID_DISTANCE,
		ASTEROID_INIT_VELOCITY_NORMAL, ASTEROID_INIT_VELOCITY_RADIAL, ASTEROID_SPRITES,
		EXPLOSION_FRAMES, EXPLOSION_PERIOD, EXTINCTION_RATE, MENU_WIDTH, N_ASTEROIDS, RECIPES,
...
pub fn asteroid_spawner(
	...
	if timer.0.tick(time.delta_seconds()).just_finished() {
		...
		let position = vec2_from_radial(
			rng.gen_range(ASTEROID_DISTANCE),
